...
export async function fetchQueue(guildId: string): Promise<QueueData> {
  const res = await apiFetch(`/api/queue?guild_id=${encodeURIComponent(guildId)}`);
  if (!res.ok) throw new Error(`Unexpected status ${res.status}`);
...
export async function skipTrack(guildId: string): Promise<QueueData> {
  const res = await apiFetch(`/api/queue/skip?guild_id=${encodeURIComponent(guildId)}`, { method: 'POST' });
  if (!res.ok) throw new Error(`Unexpected status ${res.status}`);
...
export async function clearQueue(guildId: string): Promise<void> {
  const res = await apiFetch(`/api/queue/clear?guild_id=${encodeURIComponent(guildId)}`, { method: 'POST' });
  if (!res.ok) throw new Error(`Unexpected status ${res.status}`);
}
...
export async function fetchPlayback(guildId: string): Promise<PlaybackData> {
  const res = await apiFetch(`/api/playback?guild_id=${encodeURIComponent(guildId)}`);
  if (!res.ok) throw new Error(`Unexpected status ${res.status}`);
...
export async function pausePlayback(guildId: string): Promise<void> {
  const res = await apiFetch(`/api/playback/pause?guild_id=${encodeURIComponent(guildId)}`, { method: 'POST' });
  if (!res.ok) throw new Error(`Unexpected status ${res.status}`);
}

/** Resume playback for a guild. */
export async function resumePlayback(guildId: string): Promise<void> {
  const res = await apiFetch(`/api/playback/resume?guild_id=${encodeURIComponent(guildId)}`, { method: 'POST' });
  if (!res.ok) throw new Error(`Unexpected status ${res.status}`);
}
...
export async function stopPlayback(guildId: string): Promise<void> {
  const res = await apiFetch(`/api/playback/stop?guild_id=${encodeURIComponent(guildId)}`, { method: 'POST' });
  if (!res.ok) throw new Error(`Unexpected status ${res.status}`);
}

/** Add a track by URL to the guild queue. */
export async function addToQueue(guildId: string, url: string): Promise<void> {
  const res = await apiFetch(`/api/queue/add?guild_id=${encodeURIComponent(guildId)}`, {
    method: 'POST',
